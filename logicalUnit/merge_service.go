...
func merge(population []*Individual, foodSources []*FoodSource) {
	...
	for len(populationFreeArray) != 0 {
		...
		fmt.Println("Rand1 ", foodRandomId)
		var individuaRandomId = rand.Intn(len(populationFreeArray))
		var plebId = populationFreeArray[individuaRandomId]
		fmt.Println("Rand2 ", individuaRandomId)
		var pleb = population[plebId]
		fmt.Println(populationFreeArray)
		if foodSources[foodSourceId].occupied == 0 {
			...
			foodSources[foodSourceId].occupied++
			fmt.Println(len(foodSources[foodSourceId].occupiedBy))
		...
		populationFreeArray = append(populationFreeArray[:individuaRandomId], populationFreeArray[individuaRandomId+1:]...)
